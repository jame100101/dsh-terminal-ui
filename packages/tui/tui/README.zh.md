...
- **忙碌状态界面**：实时 Thinking 与 compaction 标题使用灰阶 braille shimmer。已完成的 `✓ Thinking` 标题为灰色，展开正文使用终端默认前景色并降低亮度。成功 tool 行使用 `#7FB77E`，model / busyEnter 标题栏使用 `#7F83C6`，权限警告和 settings 选中行使用 `#C9B84A`，full access 使用 `#C75C67`。普通助手正文使用终端默认前景色，effort 与相邻计数器使用相同的终端默认前景色和 dim 样式。`/trajectory` 中模型轮次为蓝色、工具活动为红色、用户输入为青色。状态栏的活动文字及前导图标在忙碌和 idle 时都使用精确的 `#61D6D6` TrueColor；忙碌星形字形（`✶✸✹✺`）在有 live 输出时每 250ms 更新一次，静默等待工具时每 400ms 更新一次。todo dock 整行使用 `#8A8A8A`，goal dock 整行使用 `#61D6D6`。紧凑欢迎卡片使用 `#A99B45`。物理 composer 光标在 busy 时使用不闪烁的竖线，idle 或退出时恢复终端默认样式，不会在长工具输出期间持续闪烁。用户提示使用灰色块背景（Grok `bg = light`），上下各保留一行空白。助手 Markdown 的块之间保留一行空白，think/tool/assistant 节点保持相邻。
...
- **`/settings` 五个分页**：采用 Claude Code 风格界面（搜索框、可点击 tab 条、Tab / ←→ 循环切换、Esc 关闭），包括 general（busyEnter Queue/Steer、Thinking 默认显示方式，实时持久化到 `$DSH_HOME/settings.yaml` 的 `tui` namespace）、models（以 cyan 显示 provider 标题、默认选项、value-free credential 行；使用遮罩输入与确认步骤写入 `ctx.credentials`）、plugins（稳定 Loader 叶级行按 bare id 去重；每行均可选中，`Enter` 仅修改根 profile patch 可定址的目标，preset 所有权、平台条件、当前 TUI 或活跃依赖方会在 notice 栏立即显示原因；可改项使用原子写入并等待 Loader 完成切换，关闭项变暗；热应用失败时，如果后续编辑尚未替换本次写入，则恢复本次写入）、inventory（settings namespace、credential ref 与 inspect provider）和 presets（Agent preset；启动时解析并挂载清单默认项；`Enter` 使用与 Web 相同的机制，在空白 session 内串行重新组合；prompt admission 在 `turn/start` 前固定 preset；当前 preset 以 `●` 标记，损坏项变暗）。
- **`/jobs` 面板**：显示按 owner 隔离的 registry 行（当前 Agent 的 job 加未拥有的 job；id/kind/label/status/elapsed/detail），由 `onJobsChanged` 实时刷新，并在面板打开时用一秒 tick 更新耗时；其他 Agent 的 job 保持隐藏，Enter 继续通过当前 Agent 的 owner fence 请求 kill。**`/subagents` 面板**：显示 durable descendant tree（按 depth 缩进，并含 mode/activity/label 与 diagnostics）。**`/workflows` 面板**：仅从当前父 Session 的 durable `tool-workflow/*` 记录重建 run 行（status/已记录 phase/agent-count）。process-global workflow 事件不含父 Session，因此不会更新该面板。
- **`/model`** 打开 models settings 页（按 Enter 选择默认项）。**`/sessions`** 首次打开时加载 live agent 和最新 50 个持久化 session（标题/筛选），不会在启动期间扫描持久化日志；按 Enter 会完整 replay 历史，并恢复目标日志的 effective preset、最新 canonical request route 与 reasoning。surface 同一时刻只显示一个 session，而 replacement Agent 及其精确记录 route 会在旧 handle dispose 前完成解析。route 解析、composition 或 replay 失败时，当前 surface 保持原状。`/fork` 从所选 seed boundary 解析 preset 与 request route，跨过 durability flush 后产出 cold resumable artifact。面板刷新彼此隔离：`/jobs` 的一秒 tick 只读取按 owner 隔离的 job registry，session/title 读取只在打开 `/sessions` 时执行。**`/new`** 启动 fresh session。**`--continue` / `-c`** 通过 `$DSH_HOME/tui/session-recency.json` 选择当前 cwd 中最后一次在 TUI 前台使用的持久化会话，而不是选择创建时间最新的会话；索引默认保留 1,000 个会话生命周期记录，可用 `sessionRecencyMaxEntries` 调整上限。某个 cwd 尚无匹配记录时，仅按创建时间迁移顶层会话，因此新建的后台 subagent 不会接管继续目标。
