...
- **`/jobs` 面板**：显示按 owner 隔离的 registry 行（当前 Agent 的 job 加未拥有的 job；id/kind/label/status/elapsed/detail），由 `onJobsChanged` 实时刷新，并在面板打开时用一秒 tick 更新耗时；其他 Agent 的 job 保持隐藏，Enter 继续通过当前 Agent 的 owner fence 请求 kill。**`/subagents` 面板**：显示 durable descendant tree（按 depth 缩进，并含 mode/activity/label 与 diagnostics）。**`/workflows` 面板**：从当前父 Session 的 durable `tool-workflow/*` 记录重建 run 行（status/phase/log/agent-count）；process-global phase/log 事件只为 durable 证明由当前 Session 拥有且仍 active 的 run 提供临时显示更新。
- **`/model`** 打开 models settings 页（按 Enter 选择默认项）。**`/sessions`** 首次打开时加载 live agent 和最新 50 个持久化 session（标题/筛选），不会在启动期间扫描持久化日志；按 Enter 会完整 replay 历史，并恢复目标日志的 effective preset、最新 canonical request route 与 reasoning。surface 同一时刻只显示一个 session，而 replacement Agent 会在旧 handle dispose 前完成准备。`/fork` 从所选 seed boundary 解析 preset 与 request route，跨过 durability flush 后产出 cold resumable artifact。面板刷新彼此隔离：`/jobs` 的一秒 tick 只读取按 owner 隔离的 job registry，session/title 读取只在打开 `/sessions` 时执行。**`/new`** 启动 fresh session。**`--continue` / `-c`** 通过 `$DSH_HOME/tui/session-recency.json` 选择当前 cwd 中最后一次在 TUI 前台使用的持久化会话，而不是选择创建时间最新的会话；索引默认保留 1,000 个会话生命周期记录，可用 `sessionRecencyMaxEntries` 调整上限。某个 cwd 尚无匹配记录时，仅按创建时间迁移顶层会话，因此新建的后台 subagent 不会接管继续目标。
