...
- **Busy chrome**: the live Thinking row leads with a braille spinner in a stable magenta; compaction uses a yellow spinner row; neither row sweeps hues. `/trajectory` colors model turns blue, tool activity red, and user input cyan. The status-bar activity text and its leading glyph use the exact `#61D6D6` TrueColor while active or idle; the busy star glyph (`✶✸✹✺`) advances every 250ms with live output and every 400ms during a quiet tool wait. The complete todo dock row uses `#8A8A8A`, while the complete goal dock row uses `#61D6D6`. The compact welcome card uses `#A99B45`. The physical composer caret becomes a steady bar while busy and restores the terminal default on idle or exit, so it does not blink through long tool output. User prompts use a gray block background (Grok `bg = light`) plus one blank row above and below. Assistant markdown keeps one blank row between blocks; think/tool/assistant nodes stay flush.
