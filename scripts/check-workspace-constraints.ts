...
/** Private workspaces whose release artifact is assembled into another manifest. */
const stagedReleaseWorkspaceDirectories = new Set(['apps/tui-cli'])
...
export function checkWorkspaceManifest({ dir, manifest }: WorkspaceManifest): string[] {
  ...
  } else if (releaseMemberDirectory.test(dir) && !stagedReleaseWorkspaceDirectories.has(dir)) {
