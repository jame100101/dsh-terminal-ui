...
`/copy` 复制最近一条助手回复；`/copy n` 是第 n 条最近回复（Grok 编号）。`/rate up|down [n]` 使用同一倒序回复编号，并切换现有的耐久 message-feedback sidecar。提示词和回复靠拖选复制，不是靠单击。没有 `/select`，没有 `Ctrl+Y` Copy Mode，应用运行期间鼠标跟踪保持开启。Tab、方向键、Space 和字母键都不会进入消息选择状态。输入框有选区时 `Ctrl+C` 复制该选区，否则仍是取消 / 连按退出。线性/print 模式不实现剪贴板快捷键。
...
评价快捷键也采用同一结论：`/rate` 明确表示动作和目标，不占用普通编辑按键，也不增加 transcript 选择状态。
