...
`/copy` copies the latest assistant reply; `/copy n` is the Nth-latest reply (Grok numbering). `/rate up|down [n]` addresses the same reverse-chronological reply order and toggles the existing durable message-feedback sidecar. Prompts and replies are copied by dragging, not by a bare click. There is no `/select`, no `Ctrl+Y` Copy Mode, and mouse tracking stays on while the app runs. Tab, arrows, Space, and letter keys never enter a message-selection state. `Ctrl+C` copies the composer selection when one exists; otherwise it stays cancel / double-press exit. Linear/print mode does not implement clipboard shortcuts.
...
The same rejection applies to rating shortcuts: `/rate` names the action and target without reserving ordinary editor keys or adding transcript selection state.
...
Wheel and scrollbar keep working except during an active text drag, which ignores the wheel and instead auto-scrolls when the pointer holds against the transcript edge. A click on a prompt or reply does not select or copy it. A drag copies the highlighted span, auto-writes the clipboard, and clears the inverse. `/copy` and `/rate` only target assistant replies.
