...
`packages/tui/tui/tests/wrap.spec.ts`, `ui-publish.spec.ts`, `busy-star.spec.ts`, and `render-frame.spec.ts` cover incremental wrap, coalescing, dark remap, prompt vpad, the busy star, and the Thinking grayscale sweep.
